...
- **PaddleOCR 本地 OCR**：在设备本地快速识别文字。
- **Custom AI 配置档**：支持 OpenAI 兼容的翻译端点，并可选用 Custom AI OCR 处理图像型提供商。

仓库中仍可能保留旧提供商模块和历史资源，用于兼容或迁移参考；当前受支持的运行路径是 PaddleOCR 加 Custom AI 配置档。

## 主要功能

- 自选屏幕区域作为 OCR 输入和译文叠加输出
- 实时 OCR 与翻译循环，并防止过期结果覆盖新结果
- 可配置语言、模型大小、设备、置信度阈值与检测限制的 PaddleOCR 后端
...
发布版不需要另行安装 OCR 引擎。源码用户安装依赖后，可在应用设置中配置 PaddleOCR 或 Custom AI。
...
2. 选择 OCR 文字区域。
3. 选择译文叠加显示区域。
4. 在设置中选择 PaddleOCR，或配置 Custom AI OCR 配置档。
5. 配置用于翻译的 OpenAI 兼容 Custom AI 配置档。
6. 点击 **Start** 开始翻译。
...
请使用 `ocr_translator_config.example.ini` 作为可公开的配置模板。
